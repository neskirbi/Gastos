	...
	if($currentPassword == $result['password']) {
		...
		if($newPassword == $conformPassword) {
			...
			if($connect->query($updateSql) === TRUE) {
				...
				$valid['success'] = false;
				$valid['messages'] = "Error cuando se actualiza el password";	
			}

		} else {
			$valid['success'] = false;
			$valid['messages'] = "el password no coincide con el registrado";
		}

	} else {
		$valid['success'] = false;
		$valid['messages'] = "Password actual es incorrecto";
	}
