...
<page backtop="15mm" backbottom="15mm" backleft="15mm" backright="15mm" style="font-size: 12pt; font-family: arial" >
	...
	<table cellspacing="0" style="width: 100%;  text-align: left; font-size: 9.5pt;padding:1mm;">
    <?php
			setlocale(LC_TIME, 'spanish');

		//$cheques, ejmplo '1', '2'
		$filtro = "";
		if(strpos($cheques, "-") == true)
		{
			$cheques="'".str_replace("-", "' and '", $cheques)."'";
			$filtro="between ".$cheques;
		}else
		
		if(strpos($cheques, ",") == true)
		{
			$cheques="'".str_replace(",", "','", $cheques)."'";
			$filtro="in (".$cheques.")";
		}else
		{
			$filtro="='".$cheques."'";
		...
		$sql = "SELECT che.fecha, case when che.bennombre = '0' then (select name from user where id=che.beneficiario) when che.bennombre != '0' then che.bennombre end ben, che.monto, che.concepto, che.beneficiario, (select name from programas where id=che.programa) as programa, (select name from t_cheque where id=che.t_cheque) as t_cheque, (select name from user where id=che.beneficiario) as solicitud, che.no_cheque, che.fecha_confirm
				FROM cheques che
				where che.status='2' and che.no_cheque $filtro ";
		$sql = mysqli_query($con, $sql);
		while($row = mysqli_fetch_array($sql))
		{
			setlocale(LC_TIME, 'spanish');
			$fecha = strftime("%d %B %Y", strtotime($row['fecha_confirm']));
			...
			echo "	<tr>
					...
					<tr>
						<td style=' height: 195px; width: 50%';>
							***(".valorEnLetras($row['monto']).")***
						</td>
						<td style='width: 30%;'>
